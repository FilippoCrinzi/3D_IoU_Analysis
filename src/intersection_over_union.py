...
def align_iou(path_obj, path_trajectories, path_point_cloud_dir, num_points):
    ...
    for k in range(1, num_files):
        print(f"Frame: {k}")
        iou_bbox = align_iou_for_frame2(path_obj,path_trajectories, path_point_cloud_dir, k, num_points, False)
        print("IoU bbox:", iou_bbox)
        iou_ellipsoid = align_iou_ellipsoid_for_frame2(path_obj, path_trajectories, path_point_cloud_dir, k, num_points, False)
        print("IoU ellipsoid:", iou_ellipsoid)
        values_bbox.append(iou_bbox)
        values_ellipsoid.append(iou_ellipsoid)
        frame.append(k)
...
def compare_iou(path_obj, path_trajectories, path_point_cloud_dir, num_points, shape):
    ...
    if shape == 'bbox':
        for k in range(1, num_files):
            ...
            iou_bbox = iou_for_frame(path_obj, path_trajectories, path_point_cloud_dir, k, num_points, False)
            print("IoU:", iou_bbox)
            iou_align = align_iou_for_frame2(path_obj, path_point_cloud_dir, k, num_points, False)
        ...
        for k in range(1, num_files):
            ...
            print("IoU:", iou)
            iou_align = align_iou_ellipsoid_for_frame2(path_obj,path_trajectories, path_point_cloud_dir, k, num_points, False)
            print("IoU align:", iou_align)
            iou_align_values.append(iou_align)
...
def align_iou_for_frame2(path_obj, path_trajectories, path_point_cloud_dir, frame, num_points, visualize):
    point_cloud_files = utils.sort_directory_files(path_point_cloud_dir)
    mesh = o3d.io.read_triangle_mesh(path_obj)
    file_name = point_cloud_files[frame - 1]
    path_point_cloud = os.path.join(path_point_cloud_dir, file_name)
...
def align_iou_ellipsoid_for_frame2(path_obj, path_trajectories, path_point_cloud_dir, frame, num_points, visualize):
    point_cloud_files = utils.sort_directory_files(path_point_cloud_dir)
    mesh = o3d.io.read_triangle_mesh(path_obj)
    file_name = point_cloud_files[frame - 1]
    path_point_cloud = os.path.join(path_point_cloud_dir, file_name)

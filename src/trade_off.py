...
def trade_off(path_obj, path_point_cloud_dir, frame, path_trajectories, shape):
    ...
    if(shape == 'bbox'):
        ...
        utils.plot_trade_off_time(points, times, times_align)
    elif(shape == 'ellipsoid'):
        for num_points in num_point:
            start_time = time.time()
            iou_value = iou.iou_ellipsoid_for_frame(path_obj, path_trajectories, path_point_cloud_dir, frame, num_points, False)
            end_time = time.time()
            t = end_time - start_time
            times.append(t)
            values_iou.append(iou_value)
            start_time = time.time()
            iou_align_value = iou.align_iou_ellipsoid_for_frame(path_obj, path_trajectories, path_point_cloud_dir, frame, num_points, False)
            end_time = time.time()
            values_align_iou.append(iou_align_value)
            t_align = end_time - start_time
            times_align.append(t_align)
        ...
        for num_points in num_point:
            start_time = time.time()
            iou_value = iou.iou_superquadric_for_frame(path_obj, path_trajectories, path_point_cloud_dir, frame,
                                                       num_points, [0.3, 0.7], False)
            end_time = time.time()
            t = end_time - start_time
            times.append(t)
            values_iou.append(iou_value)
            start_time = time.time()
            iou_align_value = iou.align_iou_superquadric_for_frame(path_obj, path_trajectories, path_point_cloud_dir,
                                                                   frame, num_points, [0.3, 0.7], False)
            end_time = time.time()
            values_align_iou.append(iou_align_value)
            t_align = end_time - start_time
            times_align.append(t_align)
...
if __name__ == "__main__":
    ...
    trade_off(path_mercedes, point_cloud_dir, 15, trajectories, "superquadric")
